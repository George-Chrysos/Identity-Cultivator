 ...
 * Handles the two registered paths (Tempering, Presence); falls back to a
 * sensible curve for any future path whose constants we haven't wired yet.
 */
  ...
  return (
    ...
    >
      {SEED_AXES.map((axis, idx) => {
        ...
        return (
          <div key={axis} className="flex items-center gap-3 sm:gap-4">
            <SeedMedallion
              axis={axis}
              level={level}
              currentXP={currentXP}
