...
const CURRENT_SYNC_VERSION = '1.0.0'; // Increment this when you make changes
...
/**
 * Sync path metadata (main path entry)
 */
const syncPathMetadata = async () => {
...
/**
 * Sync a single level with all gates, subtasks, and trial
 */
const syncLevel = async (level: number) => {
  ...
  logger.debug(`Synced level ${level}`, { pathLevelId });
};

/**
 * Main sync function - syncs all tempering path data to database
 */
export const syncPathsToDatabase = async (): Promise<boolean> => {
  try {
    ...
    // 1. Sync path metadata
    await syncPathMetadata();

    // 2. Sync all 10 levels
    for (let level = 1; level <= 10; level++) {
      await syncLevel(level);
    }
